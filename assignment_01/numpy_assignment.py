...
def min_row_col(m):
    min_row_ix = np.argmin(np.sum(m, axis=1))
    min_col_ix = np.argmin(np.sum(np.transpose(m), axis=1))
    return [min_row_ix, min_col_ix]  
...
def was_gear_switched(car_data):
    max_value = np.max(car_data[::,1:]/car_data[::,0:1]) 
    min_value = np.min(car_data[::,1:]/car_data[::,0:1])
    if (max_value- min_value)/max_value > .1:
        return True
    return False
